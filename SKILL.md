...
3. `chat.py` submits the task and polls `chat_result` until the task is `completed` or `failed`
...
3. Read the returned JSON and use it directly:
...
5. If the user asks to continue an existing conversation, pass the previous `conversation_id`.
6. For `confirm_info` results:
...
python3 scripts/chat.py \
  --poll-interval 2 \
  --timeout 300 \
  --message "帮我生成一张小红书封面图"
```

## Guardrails

- Prefer the script over hand-written `curl` so submit and polling stay consistent.
...
- Keep `conversation_id` from the last successful response and reuse it on follow-up requests.
